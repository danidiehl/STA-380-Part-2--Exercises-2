...
destdata = abiadest[,c("lat","lon","Month")]

abialatlon <- filter(airportlatlon, IATA_FAA=="AUS") #separate df for abia

head (abiaorigin)
usa <- map_data("usa")
dest <- gather(data = destdata, -lon, -lat, key = "var", value = "value")
ggplot() + geom_polygon(data = usa, aes(x=long, y = lat, group = group)) + coord_fixed(1.3) + 
geom_curve(data=dest, aes(x = lon, y = lat, xend = abialatlon$lon, yend = abialatlon$lat), col = "turquoise", size = .01, curvature = .2) + 
