...
The graph shows the culmination of our analysis, which is the number of minutes the airport contributes to the delay. The redder routes have a slower turnaround time at the Austin airport, and blue routes have a faster turnaround time. For example, AUS was able to speed up the average turnaround tme for the flights to Des Moines, while the flights to Detroit on average was slowed down at AUS, and thus had a slower turnaround time. 
...
Overall, it seems the best way to minimize Departure Delays is to fly in September or Octover and avoid IAD as the origin airport. However, it might just be best to avoid flights involving any ofo the 
...
```
Using Naive Bayes, we get a in-sample (train) accuracy of 75%, but our out-of-sample (test) accuracy is 50%. This is fairly accurate, considering authors can write about any number of things, and every article adds dimensionality. We prefer this model, as it is significantly more accurate than randomly guessing.
```{r}
##TESTING
library(ggplot2)
set.seed(1)
...
qplot(scores[,1], scores[,2], color=X[,802], xlab='Component 1', ylab='Component 2')
```
As you can see, PCA is an excellent tool for visuali



```{r}
library(rpart)
rpart.model <- rpart(mat.df.test$categorynb ~ .,data = mat.df.test, method = "anova")
rpart.model
